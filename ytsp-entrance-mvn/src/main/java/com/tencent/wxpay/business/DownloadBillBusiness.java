...
import com.tencent.wxpay.protocol.downloadbill_protocol.DownloadBillReqData;
import com.tencent.wxpay.protocol.downloadbill_protocol.DownloadBillResData;
import com.tencent.wxpay.service.DownloadBillService;
import com.thoughtworks.xstream.io.StreamException;

/**
 * User: rizenguo Date: 2014/12/3 Time: 10:45
 * 
 * @description 下载账单
 */
public class DownloadBillBusiness {

	public DownloadBillBusiness() throws IllegalAccessException,
	...
	private static WXLog wXLog = new WXLog(
			LoggerFactory.getLogger(DownloadBillBusiness.class));
	...
	public void run(DownloadBillReqData downloadBillReqData,
		...
		wXLog.i("对账单API返回的数据如下：");
		downloadBillServiceResponseString = downloadBillService
				.request(downloadBillReqData);

		long costTimeEnd = System.currentTimeMillis();
		long totalTimeCost = costTimeEnd - costTimeStart;
		wXLog.i("api请求总耗时：" + totalTimeCost + "ms");

		wXLog.i(downloadBillServiceResponseString);
		...
		try {
			...
			downloadBillResData = (DownloadBillResData) WXUtil
					.getObjectFromXML(downloadBillServiceResponseString,
							DownloadBillResData.class);

			if (downloadBillResData == null
					|| downloadBillResData.getReturn_code() == null) {
				setResult(
						"Case1:对账单API请求逻辑错误，请仔细检测传过去的每一个参数是否合法，或是看API能否被正常访问",
			...
			if (downloadBillServiceResponseString.equals(null)
					|| downloadBillServiceResponseString.equals("")) {
				setResult("Case4:对账单API系统返回数据为空", WXLog.LOG_TYPE_ERROR);
				resultListener
						.onDownloadBillFail(downloadBillServiceResponseString);
			} else {
				setResult("Case3:对账单API系统成功返回数据", WXLog.LOG_TYPE_INFO);
				resultListener
						.onDownloadBillSuccess(downloadBillServiceResponseString);
			}
			returnCode = "SUCCESS";
			resultCode = "SUCCESS";
		} finally {
			ReportReqData reportReqData = new ReportReqData(
					...
					WXPayConfig.ip, downloadBillReqData.getAppid(),
					downloadBillReqData.getMch_id(), appKey);
