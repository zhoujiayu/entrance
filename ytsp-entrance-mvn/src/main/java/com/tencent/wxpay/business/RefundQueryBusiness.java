...
import com.tencent.wxpay.protocol.refund_query_protocol.RefundOrderData;
import com.tencent.wxpay.protocol.refund_query_protocol.RefundQueryReqData;
import com.tencent.wxpay.protocol.refund_query_protocol.RefundQueryResData;
...
public class RefundQueryBusiness {
	...
	public RefundQueryBusiness() throws IllegalAccessException,
	...
	private static WXLog wXLog = new WXLog(
			LoggerFactory.getLogger(RefundQueryBusiness.class));
	 ...
	 * @throws Exception
	 */
	public void run(RefundQueryReqData refundQueryReqData,
			RefundQueryBusiness.ResultListener resultListener, String appKey)
			throws Exception {
		...
		refundQueryServiceResponseString = refundQueryService
				.request(refundQueryReqData);

		long costTimeEnd = System.currentTimeMillis();
		long totalTimeCost = costTimeEnd - costTimeStart;
		wXLog.i("api请求总耗时：" + totalTimeCost + "ms");

		wXLog.i(refundQueryServiceResponseString);

		// 将从API返回的XML数据映射到Java对象
		RefundQueryResData refundQueryResData = (RefundQueryResData) WXUtil
				.getObjectFromXML(refundQueryServiceResponseString,
		...
		ReportReqData reportReqData = new ReportReqData(
				...
				refundQueryResData.getOut_trade_no(), WXPayConfig.ip,
				refundQueryReqData.getAppid(), refundQueryReqData.getMch_id(),
				appKey);
		...
		if (refundQueryResData == null
				|| refundQueryResData.getReturn_code() == null) {
		...
		if (refundQueryResData.getReturn_code().equals("FAIL")) {
			...
			wXLog.i("退款查询API系统成功返回数据");
			// --------------------------------------------------------------------
			// 收到API的返回数据的时候得先验证一下数据有没有被第三方篡改，确保安全
			// --------------------------------------------------------------------

			if (!Signature.checkIsSignValidFromResponseString(
					refundQueryServiceResponseString, appKey)) {
			...
			if (refundQueryResData.getResult_code().equals("FAIL")) {
				WXUtil.log("出错，错误码：" + refundQueryResData.getErr_code()
						+ "     错误信息：" + refundQueryResData.getErr_code_des());
				setResult("Case4:【退款查询失败】", WXLog.LOG_TYPE_ERROR);
				...
				setResult("Case5:【退款查询成功】", WXLog.LOG_TYPE_INFO);
	...
	private void getRefundOrderListResult(String refundQueryResponseString)
		...
		List<RefundOrderData> refundOrderList = XMLParser
				.getRefundOrderList(refundQueryResponseString);
		int count = 1;
		for (RefundOrderData refundOrderData : refundOrderList) {
			WXUtil.log("退款订单数据NO" + count + ":");
			WXUtil.log(refundOrderData.toMap());
			orderListResult += refundOrderData.toMap().toString();
			count++;
		}
		wXLog.i("查询到的结果如下：");
		wXLog.i(orderListResult);
	}
